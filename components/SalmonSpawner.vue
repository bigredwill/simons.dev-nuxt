...
const spawnSalmon = () => {
  ...
  // Salmon dimensions (approximate)
  const salmonWidth = 200;
  const salmonHeight = 200;

  // Generate random position within visible page bounds
  const startX = Math.random() * (viewportWidth - salmonWidth);
  const startY =
    Math.random() * (viewportHeight - salmonHeight) + window.scrollY;
